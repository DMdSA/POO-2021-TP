        ...
        for(int i = 0; i < subs; i++){
            do {
                 ...
                 flag = (jogoFutebol.getEquipaCasa().hasJogador(titular) && jogoFutebol.getEquipaCasa().hasJogador(suplente)
                         && jogoFutebol.mesma_posicao(jogoFutebol.getEquipaCasa().getJogadorByName(titular),jogoFutebol.getEquipaCasa().getJogadorByName(suplente)));
                 if(!flag){
                     ClientView.warning("Um dos jogadores nao esta presente na equipa ou os jogadores nao jogam na mesma posicao");
                 }
            }while (!flag);

            flag = jogoFutebol.substitui(jogoFutebol.getEquipaCasa().getJogadorByName(titular),jogoFutebol.getEquipaCasa().getJogadorByName(suplente));
            if(flag){
